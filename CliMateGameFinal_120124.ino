...
char targetWord[13] = "conservation";  // Target word for final challenge
char guessedWord[13]; // Placeholder for guessed letters
...
void setup() {
    ...
    sendMainMessage("Read the classified document and find the final keyword.");
    startGame();
}
...
void startGame() {
    ...
    gameStarted = true;
    sendMainMessage("");
    sendMainMessage("Final Challenge - Water-Saving Initiative");
    sendMainMessage("It focuses on careful management and use of water resources.");
    sendMainMessage("Ensuring long-term sustainability, especially during droughts.");
...
void loop() {
    ...
    if (charEndDetected) {
        ...
        if (isGameOver()) {
            sendMainMessage("Congratulations! You have successfully finished the adventure!");
            sendMainMessage("Conserving water helps ensure there’s enough for everyone, even during droughts!");
            sendMainMessage("Without conservation efforts, future droughts could lead to severe water shortages.");
            sendMainMessage("Our field agents are dispatched to oversee simple and sustainable water conservation,");
            sendMainMessage("that are initiated in city-wide scale to help reduce drought impact.");
            sendMainMessage("");
            sendMainMessage("Remember, human body is 70% water, conserving water is conserving your future.");
            sendStatusMessage("Adventure ended. Stay alert, see you in other adventure!");
            sendMorseMessage("-.../-.--/.");
