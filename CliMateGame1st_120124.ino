...
char targetWord[10] = "reservoir";  // Target word for first challenge
char guessedWord[10]; // Placeholder for guessed letters
...
void setup() {
    ...
    sendMainMessage("Read the classified document and find the first keyword.");
    startGame();
}
...
void startGame() {
    ...
    gameStarted = true;
    sendMainMessage("");
    sendMainMessage("1st Challenge - Water Sources");
    sendMainMessage("It is the most stable and significant water source.");
    sendMainMessage("There are currently 19 of these sources being used for New York City.");
    sendMainMessage("It can store a large volume of water.");
...
void loop() {
    ...
    if (charEndDetected) {
        ...
        if (isGameOver()) {
            ...
            sendMainMessage("Reservoirs not only store water but also help in flood control, irrigation, and recreation!");
            sendMainMessage("Polluted reservoirs can cause waterborne diseases and disrupt ecosystems.");
            sendMainMessage("Our field agents are dispatched to search for a promising additional “reservoir”.");
